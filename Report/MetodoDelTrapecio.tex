...
\section{Definiciones y resultados previos} \label{sec:previo}
	...
	Por tanto, si el orden del método es $O(h^r)$ con $r > 0$, es claro que el método es convergente ya que el error global en cualquier punto tiende a $0$ cuando $n \rightarrow +\infty$. Sin embargo, en la práctica el método puede no converger por culpa de los errores de redondeo. Se incidirá en ésto en la Sección \ref{sec:trapecio-explicito:redondeo}
...
En este último ejercicio se propone un algoritmo como la suma de Kahan. En el análisis númerico, la suma de Kahan reduce significativamente el error numérico del total. Para ello, esto lo termina rodrigo....
%-----------------------------------------------------------------------------------------------------

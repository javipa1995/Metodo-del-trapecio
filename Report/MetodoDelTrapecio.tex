...
\newcommand{\docabstract}{El objetivo de este trabajo es el de estudiar uno de los métodos numéricos parala resolución de ecuaciones diferenciales ordinarias: el método del Trapecio. Es un método didáctico y que da buenas aproximaciones de las soluciones.}
...
donde a $\epsilon_0$ se le denomina perturbación del problema inicial. Cuando se aplica un método al problema perturbado, el error introducido inicialmente puede ir aumentando en cada iteración. Si se considera el error cometido en $t_n = b$, este puede incluso diverger cuando $n \rightarrow +\infty$. Por lo tanto, para poder utilizar cualquier valor de $h$ interesa que el método a utilizar sea estable ante perturbaciones. Esto es, si se perturba la condición inicial, las diferencias entre las aproximaciones obtenidas en ambos problemas están acotadas independientemente del número de puntos que se utilicen. Este concepto se formaliza en la siguiente definición.
    ...
    \begin{proof}
        En el caso del método de Euler $\phi(t,y,h) = f(t,y)$. Por tanto, se cumplen las hipótesis del Teorema \ref{theorem:stability}. Además, el  del método de Euler es 1, luego es convergente y, por tanto, consistente.
    \end{proof}
...
Se realiza ahora el mismo esquema anterior, pero ahora con el método del Trapecio Explícito e Iterativo:
...
Se expone a continuación una gráfica comparativa de las tres aproximaciones:
...
Además, el error absoluto en el primer método es $2,235666$ obtenido mediante el valor absoluto de la diferencia de la solución calculada menos la solución evaluada en $2$, esto es, $|y_{10} - y(2)|$. De la misma forma, se obtiene el error al usar el método del Trapecio Explícito. En este caso, el error cometido es $0,199054$, por lo que se puede ver que este método es mejor que el de Euler y realizando un proceso análogo se obtiene que el error para el método del Trapecio Iterativo es 0.047213.
...
La resolución de numérica de problemas de valores iniciales puede ser una tarea compleja. Sin embargo, disponemos de métodos como el método de Euler que nos permite calcular una aproximación a la solución. Pese a ello, el coste computacional es muy grande y el método del Trapecio, ya se el explícito o el iterativo nos ayuda a conseguir dicha aproximación de forma más rápida y con un menor coste.

Además, el método del trapecio tiene un error local de orden tres y, en consecuencia, el error global es de orden dos. Más ventajas de este método es que es didáctico, que además da una introducción tanto a métodos implícitos (como el método del Trapecio Implícito o Iterativo) y a métodos de mayor orden que no dependen de las derivadas.
